...
# ---------- 配置区 ----------
OBJ_ROOT = "/home/oier/acgpj_change3d/build/1229"    # 替换为你的OBJ文件夹路径
OUTPUT_DIR = "/home/oier/acgpj_change3d/render_1229"  # 输出文件夹路径
# ---------- 配置区 ----------

# 相机参数（完全按照你的要求）
CAMERA_LOCATION = (200.0, -200.0, 350.0)
CAMERA_ROTATION = (
    math.radians(54.736),
    math.radians(-0.000003),
    math.radians(45.0)
)
CAMERA_CLIP_END = 800.0  # 裁剪距离500米
...
def modify_materials_alpha(target_alpha=0.2):
    """
    将名称包含 Water 的材质替换为玻璃 BSDF，设置微弱蓝色、粗糙度 0.1、IOR 1.33。
    保留其他材质不变。target_alpha 参数不再使用，仅为兼容旧调用。
    """
    ...
    for material in bpy.data.materials:
        ...
        # 设置玻璃参数：微蓝、低粗糙、正确折射率
        glass_node.inputs['Color'].default_value = (0.6, 0.8, 1.0, 1.0)  # 微弱蓝色
        glass_node.inputs['Roughness'].default_value = 0.1
        glass_node.inputs['IOR'].default_value = 1.33

        # 移除旧的 Surface 连接，确保只使用玻璃 BSDF
        ...
        print(f"  已将材质 '{material.name}' 设置为玻璃 BSDF")
