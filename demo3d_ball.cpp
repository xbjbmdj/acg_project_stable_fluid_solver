...
int main() {
  ...
  rigidBody.setMass(
      50000.0); // Example mass，如果mass=27，体积=27m^3，则密度和水完全一样
              // set body-space inertia for a 3m cube mass=10kg: I ~ 15 kg·m^2
              // on diag  如果半径为9.4，体积大约为4000
  change3d::Mat3 Ibody(18000000.0);//默认质量1000对应40000
  rigidBody.setInertiaBody(Ibody);
  rigidBody.setPosition(change3d::Vec3(
      60.0, 150.00, 60.0)); // Example initial position of the center
  rigidBody.setLinearVelocity(
      change3d::Vec3(0.0, -30.0, 0.0)); // Example velocity
  ...
  auto applyFreeSlip = [&](Fluid &F) {
   ...
   // std::cin>>maxi>>maxj>>maxk;
    ...
    for (int i = 1; i <= F.numX-2; i++) {// 60, 50, 60
      for (int j = 1; j <= 100; j++) {
        for (int k = 1; k <= F.numZ-2; k++) {
  ...
  for (int step = 0; step < max_step; ++step) {
    ...
    if (step % 1 ==0) {
      ...
      oname << "011307ball_250_" << step << ".obj";
...
// mkdir -p build/0113longrun1 && find build -maxdepth 1 -type f -name '011302_150_*' -exec mv {} build/0113longrun1/ \;
/*
mkdir -p tmp_seq
i=1
for f in $(printf "%s\n" 011302_150_*_render.png | sort -V); do
...
//clang-format -style=google -i fluid3d.h