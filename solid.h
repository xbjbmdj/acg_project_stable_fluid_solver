...
// Load and cache the bunny mesh from bunny_200.obj; vertices are centered at the mesh centroid.
inline const BunnyMesh& getBunnyMesh(){
    static BunnyMesh mesh;
    if(mesh.loaded) return mesh;

    const std::string path = "bunny_200.obj"; // assumes working directory contains the file
    std::ifstream in(path);
    if(!in.is_open()){
        // leave mesh empty but marked loaded to avoid repeated I/O; inside checks will fail fast
        mesh.loaded = true;
        mesh.bbMin = Vec3();
        mesh.bbMax = Vec3();
        mesh.centroid = Vec3();
    ...
    std::string line;
    mesh.bbMin = Vec3(std::numeric_limits<double>::max(), std::numeric_limits<double>::max(), std::numeric_limits<double>::max());
    mesh.bbMax = Vec3(-std::numeric_limits<double>::max(), -std::numeric_limits<double>::max(), -std::numeric_limits<double>::max());
...
inline bool rayHitsTrianglePosX(const Vec3& p, const Vec3& dir, const std::array<int,3>& tri, const std::vector<Vec3>& verts){
    // Moller-Trumbore for ray p + t*dir, dir assumed normalized-ish; returns true if t>0 intersection
...
inline bool pointInsideBunny(const Vec3& pLocal){
    ...
    if(pLocal.x < mesh.bbMin.x || pLocal.x > mesh.bbMax.x ||
       ...
       pLocal.z < mesh.bbMin.z || pLocal.z > mesh.bbMax.z) return false;
...
private:
    ...
    void resolveContainerCollisions(double dt){
        ...
        // assume no x, z walls
        m_pos.x += m_linVel.x * dt; m_pos.z += m_linVel.z * dt;
        ...
        std::cout<<"Rigid Body minY0: "<< minY0 <<", floorY: "<< floorY <<", pos.y: "<< m_pos.y <<", vy: "<< vy <<"\n";
...
bool RigidBody::is_inside(double x, double y, double z) const {
    // Transform world point into body-local coordinates; body origin assumed at bunny centroid.
    Vec3 p(static_cast<double>(x), static_cast<double>(y), static_cast<double>(z));
    Vec3 rel = p - m_pos;
    Mat3 R = m_ori.toMat3();
...
inline void RigidBody::setContainerBounds(const Vec3& minCorner, const Vec3& maxCorner){
    m_containerMin = minCorner;
    m_containerMax = maxCorner;
    m_hasContainer = true;
}

inline void RigidBody::clearContainerBounds(){
    m_hasContainer = false;
}

inline void RigidBody::setCollisionResponse(double restitution, double friction){
